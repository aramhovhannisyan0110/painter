...
(function ($) {
    ...
    $.fn.drawr = function (action, param) {
        ...
        plugin.bind_draw_events = function () {
            ...
            $(self).data("lastx", null);
            $(self).data("lasty", null);
            $(self).parent().on("touchstart.drawr", function (e) {
                e.preventDefault();
            });//cancel scroll.
            ...
            self.drawMove = function (e) {
                ...
                if ($(self).data("is_drawing") == true && plugin.check_ignore(e) == false) {
                    ...
                    for (var i = stepSize; i < dist; i += stepSize) {
                        let x = lastSpot.x + (Math.sin(angle) * i);
                        let y = lastSpot.y + (Math.cos(angle) * i);
        ...
        this.each(function () {
            ...
            if (action === "start") {
                ...
                //brush dialog
                currentCanvas.$brushToolbox = plugin.create_toolbox.call(currentCanvas, "brush", {
                    left: $(currentCanvas).parent().offset().left,
                    top: $(currentCanvas).parent().offset().top
                }, "Brushes", 80);

                $.fn.drawr.availableBrushes.sort(function (a, b) {
                ...
                currentCanvas.$undoButton.css("opacity", 0.5);
                //color dialog
                currentCanvas.$settingsToolbox = plugin.create_toolbox.call(currentCanvas, "settings", {
                    left: $(currentCanvas).parent().offset().left + $(currentCanvas).parent().innerWidth() - 80,
                    top: $(currentCanvas).parent().offset().top
                }, "Settings", 80);
                ...
                //size dialog
                //zoom dialog
                currentCanvas.$zoomToolbox = plugin.create_toolbox.call(currentCanvas, "zoom", {
                    left: $(currentCanvas).parent().offset().left + $(currentCanvas).parent().innerWidth() - 80,
                    top: $(currentCanvas).parent().offset().top
                }, "Zoom", 80);
...
jQuery.fn.drawr.register({
    ...
    drawStart: function (brush, context, x, y, size, alpha, event) {
        context.globalCompositeOperation = "source-over";
        brush.dragStartX = null;
        brush.scrollStartX = null;
        brush.dragStartY = null;
        brush.scrollStartY = null;
